 ...
 * Solution:
 * O(2 ^ N)
 * A full binary tree can only be generated for a case with odd number of nodes.
 * Start with a base case - when only one node is given a full binary tree can be formed with only one
 * node. Cache this value in a hashmap.
 *
 * Now, iteratively generate a list of possible trees for 3, 5, 7 . . . N - ((N + 1) % 2) and cache
 * this in a hashmap.
 */
