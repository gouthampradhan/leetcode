 ...
 * 3 1  5
 ...
 Note: There will only be '(', ')', '-' and '0' ~ '9' in the input
 * string. An empty tree is represented by "" instead of "()".
 *
 * Solution: O(N ^ 2) Form a node for every number and treat the first sub-string within round braces as left node and
 * the second sub-string within the round braces as the right node. Recursively solve each substring within the round
 * braces.
 */
