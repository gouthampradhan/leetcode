 ...
 * 100,000.
 *
 * Solution O(N): Start with a base case for a single days attendance record which is 'A', 'P' and 'L' so, in total
 * there are three possible records. Now, keep adding a new possible attendance record to all the above three
 * possibilities. Now, since we cannot have a combination such as 'AA' we can maintain a variable with count of
 * combinations which already has one 'A' in it and similarly maintain a combination which already has two 'L's it it,
 * only one 'L' and only 'P' in it - keep incrementing the count of each of the variables when a new combination
 * belongs to any one of this. Continue upto n and sum up the all the variables and return the answer % 10 ^ 9 + 7
 */
