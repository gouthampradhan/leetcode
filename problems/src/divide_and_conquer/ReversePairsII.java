 ...
 * Solution: O(N log N) Given two sorted arrays A[] and B[] it is quite easy to see for every element i in A, how
 * many elements in A have a value > 2 * B[j] using binary search (also possible using two pointers) - using this idea
 * we can implement standard merge sort algorithm and for every sorted pairs A[] and B[] before we merge we can total
 * number of elements in A which are > 2 x B[i]
 *
 */
