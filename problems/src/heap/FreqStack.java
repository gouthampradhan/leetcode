 ...
 * push O(log N)
 * pop O(log N)
 * Maintain a priority queue with a FreqNode where each FreqNode contains a frequency and a stack
 * with (value and pushCount). Each stack in a priority queue contains same set of values. Every time when a value
 * is to be added to the stack a new Node is created and pushed to stack which contains the push value and pushCount.
 *
 * Example: For the below push operation
 * push 5, push 5, push 5, push 6, push 6, push 7
 * the state of priority stack will be
 * 3 : 5(0) -> 5(1) -> 5(2)
 * 2 : 6(3) -> 6(4)
 * 1 : 7(5)
 *
 * When a push operation is invoked we have to identify in which stack the value has to go in
 ...
 *
 * When a pop operation is invoked remove the FreqNode from the top of the priority queue and pop the
